...
import classes from "./Hombre.module.scss";
...
const Hombre = () => {
    ...
    useEffect(()=>{
        ...
        .then((res)=>{
            let dataArray = res.data.filter(
                (page)=> page.parent === 5 && page.acf.product_showInHombre
            );
    ...
    return (
        <Fragment>
            <h1 className={classes.main__title}>HOMBRE</h1>
            <div className={classes.hombre}>
                {
                productData ?
                productData.map((product, i)=>{
                    return (
                        <div className={classes.hombreProduct}>
                            <Product
                            key={i}
