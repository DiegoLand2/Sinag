...
import classes from "./Pendientes.module.scss";
...
const Pendientes = () => {
    ...
    useEffect(()=>{
        ...
        .then((res)=>{
            let dataArray = res.data.filter(
                (page)=> page.parent === 5 && page.acf.product_showInPendientes
            );
    ...
    return (
        <Fragment>
            <h1 className={classes.main__title}>PENDIENTES</h1>
            <div className={classes.pendientes}>
                {
                productData ?
                productData.map((product, i)=>{
                    return (
                        <div className={classes.pendientesProduct}>
                            <Product
                            key={i}
