...
import classes from "./Mala.module.scss";
...
const Mala = () => {
    ...
    useEffect(()=>{
        ...
        .then((res)=>{
            let dataArray = res.data.filter(
                (page)=> page.parent === 5 && page.acf.product_showInMala
            );
    ...
    return (
        <Fragment>
            <h1 className={classes.main__title}>MALA</h1>
            <div className={classes.mala}>
                {
                productData ?
                productData.map((product, i)=>{
                    return (
                        <div className={classes.malaProduct}>
                            <Product
                            key={i}
