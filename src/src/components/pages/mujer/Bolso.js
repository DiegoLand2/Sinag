...
import classes from "./Bolso.module.scss";
...
const Bolso = () => {
    ...
    useEffect(()=>{
        ...
        .then((res)=>{
            let dataArray = res.data.filter(
                (page)=> page.parent === 5 && page.acf.product_showInBolso
            );
    ...
    return (
        <Fragment>
            <h1 className={classes.main__title}>BOLSOS</h1>
            <div className={classes.bolsos}>
                {
                productData ?
                productData.map((product, i)=>{
                    return (
                        <div className={classes.bolsosProduct}>
                            <Product
                            key={i}
