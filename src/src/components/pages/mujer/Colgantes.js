...
import classes from "./Colgantes.module.scss";
...
const Colgantes = () => {
    ...
    useEffect(()=>{
        ...
        .then((res)=>{
            let dataArray = res.data.filter(
                (page)=> page.parent === 5 && page.acf.product_showInColgantes
            );
    ...
    return (
        <Fragment>
            <h1 className={classes.main__title}>COLGANTES</h1>
            <div className={classes.colgantes}>
                {
                productData ?
                productData.map((product, i)=>{
                    return (
                        <div className={classes.colgantesProduct}>
                            <Product
                            key={i}
